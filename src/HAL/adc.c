...
/**
  * @brief	This function initializes the ADC2 and respective GPIOs
  * @param	None
  * @return	None
  */
void ADC2Init() {
	...
	//Configure the ADC
	LL_ADC_SetResolution(ADC2, LL_ADC_RESOLUTION_12B);					//Set ADC Base Resolution to 12bits
	LL_ADC_SetDataAlignment(ADC2, LL_ADC_DATA_ALIGN_RIGHT);				//Align Data to the Right

	LL_ADC_REG_SetTriggerSource(ADC2, LL_ADC_REG_TRIG_EXT_TIM8_TRGO);
	...
	LL_ADC_SetGainCompensation(ADC2, 0x00);
...
//	LL_ADC_SetOverSamplingDiscont(ADC2, LL_ADC_OVS_REG_CONT);
	...
	LL_ADC_SetCommonClock(__LL_ADC_COMMON_INSTANCE(ADC2), LL_ADC_CLOCK_ASYNC_DIV1);		//Set Fadc = ASYNC(PLLQ = 60M) (Fadc <= 60 MHz)
	...
	LL_ADC_SetChannelSamplingTime(ADC2, LL_ADC_CHANNEL_7, LL_ADC_SAMPLINGTIME_6CYCLES_5);	//Should give Fs = 60MHz / (6.5 + 12.5) = 3.16 MSps
...
/**
  * @brief	This function triggers an ADC2 Conversion, or starts the continuous conversion
  * @param	None
  * @return	None
  */
void ADC2Start() {
...
void ADC1_2_IRQHandler(void) {
	...
	if(LL_ADC_IsActiveFlag_EOC(ADC2) == 0x01 && LL_ADC_IsEnabled(ADC2) == 0x01) {
		AnalogInHandler(ANALOG_IN_BLOCK_B);
